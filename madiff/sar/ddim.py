...
from ldm.modules.diffusionmodules.util import make_ddim_sampling_parameters, make_ddim_timesteps, noise_like, \
    extract_into_tensor
from madiff.sar.utils import high_pass_filter

def kv_filter(qkv_injected, is_k_filtered=False, is_v_filtered=False, mask=None, ms=0.5):
    if is_k_filtered and len(qkv_injected['k']) > 0:
        for i in range(len(qkv_injected['k'])):
            n = qkv_injected['k'][i].shape[1]
            h = w = int(math.sqrt(n))
            k_i = rearrange(qkv_injected['k'][i], 'b (h w) d -> b d h w', h=h, w=w)
            k_i = rearrange(high_pass_filter(k_i, mask, ms), 'b d h w -> b (h w) d')
            qkv_injected['k'][i] = k_i
    if is_v_filtered and len(qkv_injected['v']) > 0:
        for i in range(len(qkv_injected['v'])):
            n = qkv_injected['v'][i].shape[1]
            h = w = int(math.sqrt(n))
            v_i = rearrange(qkv_injected['v'][i], 'b (h w) d -> b d h w', h=h, w=w)
            v_i = rearrange(high_pass_filter(v_i, mask, ms), 'b d h w -> b (h w) d')
            qkv_injected['v'][i] = v_i
    return qkv_injected
        ...
        if not skip_inversion:
            with torch.no_grad():
                ...
                for i, step in enumerate(iterator):
                    ...
                    if is_qkv_injected:
                        self.qkv_timesteps.append(kv_filter(self.model.model.diffusion_model.qkv_store, False, True, mask, ms))# [{'q':[], 'k':[], 'v':[]}, {'q':[], 'k':[], 'v':[]}, ...]
                    z = outs    #xTs
